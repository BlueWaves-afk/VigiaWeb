...
const CARD =
  "rounded-2xl border border-slate-800/60 bg-slate-950/80 backdrop-blur-sm p-4 light:border-slate-200 light:bg-white";
...
export default function CopilotGeoRAG() {
  ...
  function makeSmartNarrative(ctx: SimContext): string {
    ...
    if (ctx.eventsLast48h > 10) {
      narrative += ` ⚠️ HIGH RISK ZONE: ${ctx.eventsLast48h} severe incidents reported in the last 48 hours.`;
    } else if (ctx.eventsLast48h > 5) {
      narrative += ` ${ctx.eventsLast48h} incidents reported here (48h).`;
    } else {
  ...
  function simulateAlert(ctx: SimContext) {
    ...
    const copilotLine: CopilotMsg = {
      ...
      role: "copilot",
      text: narrative,
      small: `Analysis: ${ctx.eventsLast48h} incidents (48h) • ${ctx.nearbyPOIs?.length || 0} nearby POIs • Confidence: ${(
        85 + Math.random() * 15
      ).toFixed(0)}%`,
  ...
  return (
    <section className="space-y-5">
      <div className="space-y-1">
        <p className="text-xs font-medium tracking-[0.22em] text-cyan-400 light:text-blue-600">COPILOT</p>
        <h2 className="text-lg font-semibold tracking-tight text-white light:text-slate-900">VIGIA Co-Pilot</h2>
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-2xl font-bold text-white light:text-slate-900">VIGIA Co-Pilot</h2>
            </div>
      ...
      {currentPreset && (
        ...
        >
          <StatCard icon={<MapPin className="h-4 w-4" />} label="Distance" value={fmtDistance(currentPreset.distanceM)} color="blue" />
          <StatCard icon={<AlertTriangle className="h-4 w-4" />} label="Hazard Type" value={currentPreset.hazardType} color="orange" />
          <StatCard icon={<TrendingUp className="h-4 w-4" />} label="Incidents (48h)" value={`${currentPreset.eventsLast48h}`} color="red" />
          <StatCard icon={<Navigation className="h-4 w-4" />} label="Max Speed" value={`${currentPreset.recommendedSpeedKmh} km/h`} color="green" />
        </motion.div>
      )}
          ...
          <AnimatePresence initial={false} mode="popLayout">
            {msgs.map((m, idx) => (
              <motion.div
                ...
                className={`flex ${
                  m.role === "copilot" ? "justify-start" : m.role === "user" ? "justify-end" : "justify-center"
                }`}
              >
                <div
                  className={
                    m.role === "system"
                      ? "rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-center text-xs text-white/70 light:border-slate-200 light:bg-slate-100 light:text-slate-700"
                      : m.role === "user"
                      ? "group max-w-[85%] rounded-2xl rounded-tr-sm bg-gradient-to-br from-blue-500 to-blue-600 px-4 py-3 shadow-lg text-white"
                      : "group max-w-[85%] rounded-2xl rounded-tl-sm border border-emerald-400/30 bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 px-4 py-3 shadow-lg backdrop-blur-sm text-white light:border-emerald-200 light:bg-emerald-50 light:text-emerald-900"
                  }
                >
                  <div className={`leading-relaxed ${m.role === "user" ? "text-white" : "text-white/95 light:text-slate-900"}`}>
                    {m.text}
                  </div>
                  {m.small && (
                    <div className={`mt-2 flex items-center gap-2 text-[11px] ${m.role === "user" ? "text-white/70" : "text-white/60 light:text-slate-700"}`}>
                  ...
                  {m.metadata?.confidence && (
                    <div className="mt-2 flex items-center gap-2">
                      <div className="h-1 flex-1 overflow-hidden rounded-full bg-white/20 light:bg-slate-200">
                        <motion.div
                      ...
                      <span className="text-[10px] text-white/60 light:text-slate-700">
...
function StatCard({
  ...
  const colorClasses = {
    blue: "from-blue-500/20 to-blue-600/10 border-blue-400/30 text-blue-400 light:border-blue-200 light:text-blue-600",
    orange: "from-orange-500/20 to-orange-600/10 border-orange-400/30 text-orange-400 light:border-orange-200 light:text-orange-600",
    red: "from-red-500/20 to-red-600/10 border-red-400/30 text-red-400 light:border-red-200 light:text-red-600",
    green: "from-green-500/20 to-green-600/10 border-green-400/30 text-green-400 light:border-green-200 light:text-green-600",
  };

  return (
    <motion.div
      whileHover={{ scale: 1.02, y: -2 }}
      className={`rounded-xl border bg-gradient-to-br p-4 backdrop-blur-sm ${colorClasses[color]} light:bg-white`}
    >
      <div className="flex items-center gap-2 text-xs font-medium opacity-80">
        {icon}
        {label}
      </div>
      <div className="mt-2 text-xl font-bold capitalize text-white light:text-slate-900">{value}</div>
    </motion.div>
  );
}
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap gap-2">
      {tags.map((tag, idx) => (
        <motion.div
          ...
          className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/80 backdrop-blur-sm light:border-slate-200 light:bg-slate-100 light:text-slate-800"
        >
          {tag.icon}
      ...
      {ctx.alternativeRoute && (
        <motion.div
          ...
          className="flex items-center gap-1.5 rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-400 backdrop-blur-sm light:border-emerald-200 light:bg-emerald-100 light:text-emerald-700"
        >
