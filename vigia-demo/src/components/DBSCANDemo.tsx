...
function seededPoints(n = 240): Pt[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    pts.push({ id: i + 1, x: b.cx + (Math.random() - 0.5) * b.r * 2, y: b.cy + (Math.random() - 0.5) * b.r * 2 });
  }
  for (let i = 0; i < 24; i++) {
  ...
  return (
    <section className="space-y-5">
      <div className="space-y-1">
        <p className="text-xs font-medium tracking-[0.22em] text-cyan-400 light:text-blue-600">CLUSTERING</p>
        <h2 className="text-lg font-semibold tracking-tight text-white light:text-slate-900">
          DBSCAN hazard clustering
        </h2>
        <p className="text-sm text-slate-400 light:text-slate-600">
          Cluster hazard reports on the edge; noise stays isolated, dense regions become actionable hotspots.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(720px,1fr)_380px]">
        <motion.div
          ...
          className="card-glass p-4 bg-slate-950/80 border-slate-800/60 light:bg-white light:border-slate-200"
        >
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div className="text-sm text-white/80 light:text-slate-800">
              DBSCAN clustering of hazard reports (interactive)
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={resetPoints}
                className="rounded-md bg-white/10 px-3 py-1 text-sm text-white hover:bg-white/20 light:bg-slate-200 light:text-slate-900 light:hover:bg-slate-300"
              >
                Randomize
              </button>
              <button
          ...
          <div className="grid gap-4 md:grid-cols-[260px_1fr]">
            <div className="rounded-xl border border-white/10 bg-black/30 p-3 light:border-slate-200 light:bg-slate-100">
              <div className="text-xs text-white/60 light:text-slate-600">
                eps: <b className="text-white light:text-slate-900">{eps}</b> px
              </div>
              <input
                type="range"
                min={8}
                ...
                className="w-full"
              />
              <div className="mt-3 text-xs text-white/60 light:text-slate-600">
                minPts: <b className="text-white light:text-slate-900">{minPts}</b>
              </div>
              <input
                type="range"
                min={2}
                ...
                className="w-full"
              />
              <div className="mt-4 rounded-lg bg-white/5 p-2 text-xs text-white/70 light:bg-slate-200 light:text-slate-800">
                <div>• Click canvas to add points</div>
                <div>• Gray = noise; Colors = clusters; Gold ring = centroid</div>
              </div>
            </div>

            <div className="rounded-xl border border-white/10 bg-black/20 p-2 light:border-slate-200 light:bg-slate-100">
              <motion.svg
              ...
              >
                ...
                {clusters.map((c) => (
                  <g key={`c-${c.id}`}>
                    ...
                    <text x={c.cx + 10} y={c.cy - 10} fontSize={12} fill="#fde68a">
                      {`C${c.id} (${c.size})`}
                    </text>
                  </g>
                ))}
        ...
        >
          <div className="card-glass p-4 bg-slate-950/80 border-slate-800/60 light:bg-white light:border-slate-200">
            <div className="mb-2 flex items-center justify-between">
              <div className="text-sm font-semibold text-white light:text-slate-900">DBSCAN — What, How, Why</div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setSlide((s) => Math.max(0, s - 1))}
                  className="rounded-md bg-white/10 px-2 py-1 text-xs text-white hover:bg-white/20 light:bg-slate-200 light:text-slate-900 light:hover:bg-slate-300"
                >
                  Prev
                </button>
                <button
            ...
            <div className="relative h-[240px] overflow-hidden">
              <AnimatePresence mode="wait">
                {slide === 0 && (
                  ...
                  >
                    <div className="space-y-3 text-white/85 light:text-slate-800">
                      <p>
                  ...
                  >
                    <div className="space-y-3 text-white/85 light:text-slate-800">
                      ...
                      <ol className="list-decimal space-y-2 pl-5">
                        ...
                        <li>If neighbors ≥ <b>minPts</b>, create a cluster and expand via connected neighbors.</li>
                ...
                {slide === 2 && (
                  ...
                  >
                    <div className="space-y-3 text-white/85 light:text-slate-800">
                      <p>
                        <b>Why it matters for road‑hazard detection</b>:
                      </p>
                      <ul className="list-disc space-y-2 pl-5">
                  ...
                  >
                    <div className="space-y-3 text-white/85 light:text-slate-800">
                      ...
                      <ul className="list-disc space-y-2 pl-5">
                        ...
                        <li>Uses grid bucketing for fast neighbor lookups.</li>
                      </ul>
                    </div>
                  </motion.div>
                )}
            ...
            <div className="mt-3 flex gap-1">
              {[0, 1, 2, 3].map((i) => (
                <button
                  key={i}
                  onClick={() => setSlide(i)}
                  className={`h-2.5 w-2.5 rounded-full ${i === slide ? "bg-white" : "bg-white/30 hover:bg-white/50"} light:bg-slate-300 light:hover:bg-slate-400`}
                />
              ))}
            </div>
          </div>
