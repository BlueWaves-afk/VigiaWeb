...
/** Outer page only provides the Suspense boundary */
...
/** Inner component is allowed to call useSearchParams */
            ...
            <div className="relative">
              <motion.div variants={itemVariants} transition={{ duration: 0.5, ease: "easeOut" }} className="mb-8 text-center">
                <h1 className="text-4xl font-semibold tracking-tight md:text-5xl">
                  <span className="bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">Welcome back</span>
                </h1>
                <p className="mt-2 text-sm text-slate-400">Sign in to continue to VIGIA</p>
              </motion.div>

              {error && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.3 }}
              ...
              <form action={signIn} className="space-y-5">
                <motion.div variants={itemVariants} transition={{ duration: 0.5, ease: "easeOut" }}>
                  ...
                  <div className="relative">
                    ...
                    <motion.input whileFocus={{ scale: 1.01 }} transition={{ duration: 0.2 }} name="email" type="email" required
                ...
                <motion.div variants={itemVariants} transition={{ duration: 0.5, ease: "easeOut" }}>
                  ...
                  <div className="relative">
                    ...
                    <motion.input whileFocus={{ scale: 1.01 }} transition={{ duration: 0.2 }} name="password" type="password" required
                      placeholder="••••••••"
                      className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-11 pr-4 text-white placeholder-slate-500 outline-none ring-0 transition-all focus:border-sky-500/50 focus:bg-white/10 focus:ring-2 focus:ring-sky-500/20" />
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} transition={{ duration: 0.5, ease: "easeOut" }} className="pt-2">
                  <motion.button whileHover={{ scale: 1.02, x: 2 }} whileTap={{ scale: 0.98 }} type="submit"
              ...
              <motion.div variants={itemVariants} transition={{ duration: 0.5, ease: "easeOut" }}
                ...
                <motion.a whileHover={{ x: 2 }} href="/auth/forgot-password"
              ...
              <motion.div variants={itemVariants} transition={{ duration: 0.5, ease: "easeOut" }} className="mt-8 text-center">
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-xs">
                    <span className="bg-black/30 px-4 text-slate-500">New to VIGIA?</span>
                  </div>
                </div>
                <motion.a whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} href="/auth/signup"
